...
constexpr float SELF_FILTER_PADDING = 0.05f;   // 手臂安全範圍    
...
struct __attribute__((packed)) PlanRequest { 
    float start[6]; float goal[6]; uint32_t max_iter; float range; 
};
...
bool plan_path(const float start[6], const float goal[6], const std::vector<PointCloudBuffer::Point3D>& raw, uint32_t iter, float rng_val, std::vector<std::array<float, 6>>& path_out, std::vector<PointCloudBuffer::Point3D>& f_obs, std::vector<SphereData>& sph_out, uint32_t& ms) {
...
void socket_server_thread() {
    ...
    while (g_running) {
        ...
        if (bytes_read == sizeof(req)) {
            float start_aligned[6], goal_aligned[6];
            std::memcpy(start_aligned, req.start, sizeof(req.start)); std::memcpy(goal_aligned, req.goal, sizeof(req.goal));

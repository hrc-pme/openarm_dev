...
class UnityAdapterNodeWithCBF(Node):
    ...
    def _generate_cbf_constraints(self, arm, arm_pts, cloud, q_full):
        ...
        valid_indices = np.where(min_dists_to_cloud >= SELF_FILTER_RADIUS)[0]
        dangerous_pts = np.array([]) 
        if len(valid_indices) > 0:
            valid_dists = min_dists_to_cloud[valid_indices]
            k = min(MAX_CBF_POINTS, len(valid_dists))
            nearest_k_indices_in_valid = np.argpartition(valid_dists, k-1)[:k]
            original_indices = valid_indices[nearest_k_indices_in_valid]
            dangerous_pts = cloud[original_indices]
    ...
    def _compute_safe_velocity(self, arm, dq_nominal, dt):
        ...
        with self.pb_lock:
            for i, pb_idx in enumerate(self.movable_joints):
                p.resetJointState(robot_id, pb_idx, q_full[i])
            arm_pts = np.array([p.getLinkState(robot_id, i)[0] for i in current_arm_indices])

        with self.pc_lock:
            cloud = self.point_cloud.copy() if self.point_cloud is not None else None

        with self.pb_lock:
            G, h = self._generate_cbf_constraints(arm, arm_pts, cloud, q_full)
       ...
       # 穩定 dt，防止運算延遲導致步伐跳動
            ...
            with self.pc_lock:
                if self.point_cloud is not None and len(self.point_cloud) > 0:
                    self.point_cloud_visual_id = p.addUserDebugPoints(
                        pointPositions=self.point_cloud,
                        pointColorsRGB=[[1, 0, 0]] * len(self.point_cloud),
                        pointSize=3.0
                    )
        ...
        for arm in self.arms:
            ...
            if self.use_cbf and self.pybullet_ready and self.is_connected[arm]:
                ...
                dq_safe = self._compute_safe_velocity(arm, dq_nominal, dt) if dt > 0 else np.zeros(dof)

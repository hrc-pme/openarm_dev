...
// [相機設定]
constexpr float CAM_X_OFFSET = 0.35f;
constexpr float CAM_Y_OFFSET = 0.00f;
constexpr float CAM_Z_OFFSET = 0.60f;
...
private:
    void pointcloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z, ++count) {
            ...
            // 座標轉換邏輯：相機在手臂前方35cm，高度60cm，由上往下看，相機上方朝向手臂 (-X 方向)
            // Camera Z (向前) = World -Z
            // Camera -Y (相機上方) = World -X => Camera Y = World +X
            // Camera X (向右) = World +Y
            ...
            // 過濾掉高度太低（地板）以及太遠的點雲
            if (z_f > FLOOR_THRESHOLD && x_f >= -0.50f && x_f <= 0.80f && y_f >= -1.0f && y_f <= 1.0f) {
